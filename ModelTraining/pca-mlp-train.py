...
def train_model(pc_num, seq_len):
	...
	checkpoint=ModelCheckpoint(f"./mlp-pc{pc_num}_sl{seq_len}.hdf5", save_best_only=True, save_weights_only=True, mode='min')
	...
	np.save(f'loss_mlp_pc{pc_num}_sl{seq_len}.npy',loss_data)
